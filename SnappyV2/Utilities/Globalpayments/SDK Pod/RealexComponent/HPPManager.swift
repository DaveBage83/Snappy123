...
@objc public protocol HPPManagerDelegate: AnyObject {
...
public protocol GenericHPPManagerDelegate: AnyObject {
...
public class GenericHPPManager<T: Decodable>: NSObject, HPPViewControllerDelegate {
    ...
    var networkActivityHandler: ((Bool) -> ())?
    weak var globalpaymentsViewModel: GlobalpaymentsHPPViewModel?
    ...
    @objc public init(session: URLSession = .shared, globalpaymentsHPPViewModel: GlobalpaymentsHPPViewModel) {
        self.session = session
        self.globalpaymentsViewModel = globalpaymentsHPPViewModel
        super.init()
        
        //self.globalpaymentsViewModel.delegate =
...
//    }
    ...
    private func httpBodyWithJSON(_ json: NSDictionary) -> Data {
        ...
        // API v2 devs wrapped response in an object called "HPPProducerData"
        if let HPPProducerData = json["HPPProducerData"] as? [String: Any] {
            for (key, value) in HPPProducerData {
        ...
        parameters["HPP_POST_RESPONSE"] = HPPRequestProducerURL.scheme! + "://" + HPPRequestProducerURL.host!
    ...
    fileprivate func getHPPRequest() {
        ...
        let dataTask = session.dataTask(with: request, completionHandler: { data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                self.networkActivityHandler?(false)
                do {
    ...
    fileprivate func getPaymentForm() {
        ...
        globalpaymentsViewModel?.loadRequest(request)
...
//    }
    ...
    func HPPViewControllerFailedWithError(_ error: Error?) {
        ...
        //globalpaymentsViewModel?.dismissView()
    }
...
extension GenericHPPManager {
    
    func getHPPRequest(with data: Data) {
        do {
            self.HPPRequest = try JSONSerialization.jsonObject(with: data, options: []) as? NSDictionary
            if (self.isEncoded == true) {
                self.HPPRequest = self.HPPRequest.decodeAllValues()
            }
            self.getPaymentForm()
