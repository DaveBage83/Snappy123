        ...
        if let store = selectedStore, let paymentMethods = store.paymentMethods {
            ...
            if let basket = basket, basket.items.contains(where: { $0.isAlcohol }) {
                ...
                if let tempTodayTimeSlot = tempTodayTimeSlot {
                    if let cutoffTimeDateFormat = paymentMethod.settings.cutoffTime?.stringToHoursMinsAndSecondsOnly, let slotEndDateHMSOnly = tempTodayTimeSlot.endTime.hourMinutesSecondsString(timeZone: timeZone).stringToHoursMinsAndSecondsOnly {
