...
class CheckoutPaymentHandlingViewModelTests: XCTestCase {
    ...
    func test_givenBasketTimeSlotAndStoreWithCheckoutcom_whenContinueButtonTappedAndBusinessOrderIdReturned_thenCorrectCallsAreMadeAndStateSuccessful() async {
        ...
        let basket = Basket(basketToken: "", isNewBasket: true, items: [], fulfilmentMethod: .mockedData, selectedSlot: BasketSelectedSlot(todaySelected: true, start: slotStartTime, end: slotEndTime, expires: nil), savings: nil, coupon: nil, fees: nil, tips: nil, addresses: nil, orderSubtotal: 10, orderTotal: 11, storeId: nil, basketItemRemoved: nil)
    ...
    func test_givenBasketTimeSlotAndStoreWithCheckoutcomWithTestMode_whenContinueButtonTappedAndBusinessOrderIdReturned_thenCorrectCallsAreMadeAndStateSuccessful() async {
        ...
        let basket = Basket(
            ...
            selectedSlot: BasketSelectedSlot(todaySelected: true, start: slotStartTime, end: slotEndTime, expires: nil),
            savings: nil,
            coupon: nil,
            fees: nil,
            tips: nil,
    ...
    func test_givenBasketTimeSlotAndStoreWithCheckoutcom_whenContinueButtonTappedAndNilsReturned_thenCorrectCallsAndStateIsUnsuccessful() async {
        ...
        let checkoutService = MockedCheckoutService(expected: [
            .processNewCardPaymentOrder(fulfilmentDetails: draftOrderDetailRequest, paymentGatewayType: PaymentGatewayType.checkoutcom, paymentGatewayMode: .sandbox, instructions: nil, publicKey: selectedStore.paymentGateways?.first?.fields?["publicKey"] as? String ?? "", cardDetails: cardDetails),
            // used by the error logging:
            .currentDraftOrderId,
            .currentDraftOrderId
        ])
        let services = DIContainer.Services(
        ...
        XCTAssertTrue(setBillingTriggered)
        XCTAssertEqual(sut.draftOrderFulfilmentDetails, draftOrderDetailRequest)
        XCTAssertEqual(sut.container.appState.value.errors.first as! CheckoutPaymentHandlingViewModelError, CheckoutPaymentHandlingViewModelError.processCardOrderResultEmpty)
    ...
    func test_whenThreeDSFailTriggered_thenUrlsAreNilAndPaymentOutcomeIsUnsuccessful() {
        ...
        XCTAssertNil(sut.threeDSWebViewURLs)
        XCTAssertEqual(sut.container.appState.value.errors.first as! CheckoutPaymentHandlingViewModelError, CheckoutPaymentHandlingViewModelError.threeDSVerificationFailed)
    }
